...
static const uint64_t deep_sleep_secs = 30;
...
void app_main(void) {
    ...
    while(1) {
        ...
        switch (current_id_protocol) {
            case 0: {
                ...
                if (current_tl_protocol == 0) {
                    ...
                    vTaskDelay((deep_sleep_secs * 1000) / portTICK_PERIOD_MS);
                }
            ...
            case 1: {
                ...
                if (current_tl_protocol == 0) {
                    send(sock, (char *)&m, len ,0);
                    vTaskDelay((deep_sleep_secs * 1000) / portTICK_PERIOD_MS);
                    //esp_deep_sleep(deep_sleep_secs*1000000);
                }
                else if (current_tl_protocol == 1){
                    sendto(udp_sock, (char*)&m, len, 0,
                           (struct sockaddr *) &server_addr_udp, sizeof(server_addr_udp));
            ...
            case 2: {
                ...
                if (current_tl_protocol == 0) {
                    send(sock, (char *) &m, len, 0);
                    //esp_sleep_enable_timer_wakeup(deep_sleep_secs * 1000000);
        ...
        //Mandndo el mensaje con el mism oglobal id
        ESP_LOGI(TAG, "Enviando un REQ para sincronizar!");
        uint16_t len = 12; //<- largo a enviar!
        add_headers(&m, global_id, mac, current_tl_protocol, 20, len);
        if (current_tl_protocol == 0) {
            send(sock, (char *) &m, len, 0);
        } else if (current_tl_protocol == 1) {
            sendto(udp_sock, (char *) &m, len, 0,
